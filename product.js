const categories = [
    ...
    products.forEach(product => {
      ...
      const productCard = `
        <div class="product-card">
          <img src="${product.image}" alt="${product.title}" class="product-image" onclick="showSecondImage(this)">
          ${product.second_image ? `<img src="${product.second_image}" alt="${product.title}" class="product-image second-image">` : ''}
  ...
  function changeTab(tab, categoryContainer) {
    const category = categories.find(cat => cat.category_name.toLowerCase() === tab.toLowerCase());
    if (category) {
      renderProducts(`${tab.toLowerCase()}Products`, category.category_products);
    }
  
    // Highlight the selected tab
    const tabs = categoryContainer.querySelectorAll('.tab');
    tabs.forEach(t => t.classList.remove('active'));
    const selectedTab = categoryContainer.querySelector(`.tab[data-category="${tab}"]`);
    if (selectedTab) {
      selectedTab.classList.add('active');
    }
  }
  
  // Function to show the second image on click
  function showSecondImage(image) {
  ...
  // Dynamically create fixed tabs for each category section
  ...
  categories.forEach(category => {
    ...
    category.category_products.forEach(product => {
      ...
      tab.textContent = product.title;
      tab.dataset.category = category.category_name;
      tab.addEventListener('click', () => changeTab(product.title, categoryContainer));
    ...
    productsContainer.id = `${category.category_name.toLowerCase()}Products`;
  ...
  changeTab('Mens', document.querySelector('.category-container'));